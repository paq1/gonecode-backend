        ...
        .await
    {
        Ok(items) => {
            let paged_view: Paged<EntityView<PouetViewState>> =
                items.map(|entity| from_states_to_entity_view(entity, "pouet".to_string(), &ctx));

            HttpResponse::Ok().json(paged_view.to_many_view(
                &ctx,
                "pouet".to_string(),
                HashMap::from([("pouet".to_string(), "pouet".to_string())]),
            ))
        }
...
pub async fn fetch_one_pouet(
    ...
    match store.fetch_one(&id).await {
        Ok(Some(entity)) => {
            let view = from_states_to_view(entity, "pouet".to_string(), &ctx);
    ...
    match journal.fetch_many(query_core_with_filter).await {
        Ok(items) => {
            let paged_view = items.map(|entity_event| EntityView {
                r#type: "urn:api:pouet:pouet".to_string(),
...
#[utoipa::path(
    tag = "pouet",
    path = "/pouet/{entity_id}/events/{event_id}",
    responses(
        (
        status = 200,
